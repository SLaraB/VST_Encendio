...
#class_labels = model.classifier.in_features
...
num_features = model.classifier.in_features
model.classifier = torch.nn.Linear(num_features, num_reduced_labels)
...
resultados_folder = 'Resultados_Densenet'  # Ruta a la carpeta de resultados
...
csv_folder = 'CSVs'
csv_filename = os.path.join(csv_folder, 'densenet_results.csv')
...
with open(csv_filename, mode='w', newline='') as file:
    ...
    for i in range(len(imgs)):
        ...
        plt.close(fig)
        # Obtener el nombre del archivo de la imagen y el mapa de saliencia
...
print(f"Tiempo de ejecución Densenet: {execution_time} segundos")